...
export function assertStringValue(value: unknown, label: string): string {
  if (typeof value !== "string") {
...
export function assertNumberValue(value: unknown, label: string): number {
...
export function validateFieldAnswer(field: FormField, answer: unknown): void {
...
export function validateJoinFormAnswers(
...
export function parseJoinFormSchema(rawSchema: unknown): JoinFormSchema | null {
  if (!rawSchema) {
    return null
  }
  ...
  if (!parsed.success) {
    throw new ValidationError("Join form schema is invalid")
  }
