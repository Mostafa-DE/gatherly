import type {
  RankingDomain,
  MatchScoreValidation,
  MatchResult,
} from "../types"
import { BowlingMatchInput } from "./match-input"
import { BowlingMatchDisplay } from "./match-display"

// =============================================================================
// Score Validation
// =============================================================================

type BowlingScore = { team1: number; team2: number }

export function validateBowlingScores(scores: unknown): MatchScoreValidation {
  if (typeof scores !== "object" || scores === null) {
    return { isValid: false, error: "Scores must be an object with team1 and team2" }
  }

  const { team1, team2 } = scores as Record<string, unknown>

  if (typeof team1 !== "number" || typeof team2 !== "number") {
    return { isValid: false, error: "Both team scores must be numbers" }
  }

  if (!Number.isInteger(team1) || !Number.isInteger(team2)) {
    return { isValid: false, error: "Pins must be whole numbers" }
  }

  if (team1 < 0 || team2 < 0) {
    return { isValid: false, error: "Pins cannot be negative" }
  }

  return { isValid: true }
}

// =============================================================================
// Match Resolution
// =============================================================================

export function resolveBowlingMatch(scores: unknown): MatchResult {
  const { team1, team2 } = scores as BowlingScore

  const winner =
    team1 > team2
      ? ("team1" as const)
      : team2 > team1
        ? ("team2" as const)
        : ("draw" as const)

  const baseStats = {
    matches_played: 1,
    wins: 0,
    draws: 0,
    losses: 0,
    pins_scored: 0,
    pins_conceded: 0,
  }

  return {
    winner,
    team1Stats: {
      ...baseStats,
      wins: winner === "team1" ? 1 : 0,
      draws: winner === "draw" ? 1 : 0,
      losses: winner === "team2" ? 1 : 0,
      pins_scored: team1,
      pins_conceded: team2,
    },
    team2Stats: {
      ...baseStats,
      wins: winner === "team2" ? 1 : 0,
      draws: winner === "draw" ? 1 : 0,
      losses: winner === "team1" ? 1 : 0,
      pins_scored: team2,
      pins_conceded: team1,
    },
  }
}

// =============================================================================
// Domain Definition
// =============================================================================

export const bowlingDomain: RankingDomain = {
  id: "bowling",
  name: "Bowling",
  statFields: [
    { id: "matches_played", label: "Matches Played" },
    { id: "wins", label: "Wins" },
    { id: "draws", label: "Draws" },
    { id: "losses", label: "Losses" },
    { id: "pins_scored", label: "Total Pins" },
    { id: "pins_conceded", label: "Opponent Pins" },
  ],
  tieBreak: [
    { field: "wins", direction: "desc" },
    { field: "(pins_scored - pins_conceded)", direction: "desc" },
  ],
  defaultLevels: [
    { name: "Beginner", color: "#6B7280" },
    { name: "Recreational", color: "#3B82F6" },
    { name: "League", color: "#10B981" },
    { name: "Advanced", color: "#F59E0B" },
    { name: "Pro", color: "#EF4444" },
  ],
  matchConfig: {
    supportedFormats: ["singles", "doubles"],
    defaultFormat: "singles",
    formatRules: {
      singles: { playersPerTeam: 1 },
      doubles: { playersPerTeam: 2 },
    },
    validateScores: validateBowlingScores,
    resolveMatch: resolveBowlingMatch,
    MatchInput: BowlingMatchInput,
    MatchDisplay: BowlingMatchDisplay,
  },
  sessionConfig: { mode: "match" },
}
