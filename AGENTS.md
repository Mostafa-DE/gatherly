...
## 18. Plugin Schema Integration

- Plugin-owned database tables are allowed in plugin modules (for example, `src/plugins/<plugin>/schema.ts`).
- It is intentional to compose plugin DB schemas through central DB wiring (for example, `drizzle.config.ts` schema globs and `src/db/index.ts` schema composition).
- Keep shared/core DB tables in `src/db/schema.ts`; keep plugin-only DB tables in the owning plugin module.
- For validation schemas: keep shared schemas in `src/schemas/`, and keep plugin-specific schemas under plugin paths when they are plugin-only.
