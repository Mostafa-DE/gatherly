...
Do not use `python3`, `jq`, or other helper commands for encoding. This agent is curl-only.
...
2. **Check response status:**
   ...
   - If `status: "ready"`: proceed with the returned `organization`, `role`, and `supportedActions`.
     - `role` is one of `"owner"`, `"admin"`, or `"member"`.
     - Admins/owners get full `supportedActions`. Members get an empty list (no mutation capabilities yet).
...
5. Call `getSessions` to browse recent/upcoming sessions, or `searchSessions` to find sessions by name.
   - **When the user asks about a specific session by name, ALWAYS use `searchSessions`.** It searches ALL sessions (past + upcoming, any age) by title.
   - Use `getSessions` with `activityId` to narrow results to a specific activity.
   - Use `includePast: true` (default) and `pastLimit` (default 30, max 100) to control past session results.
6. Call `getParticipants` with the `sessionId` to get participant details.
7. Use `getMemberSummary` to look up a member by name/nickname when you need a userId that isn't a current participant (e.g., for `add_participant`). If multiple matches are returned, ask the user to clarify.
8. Resolve names/time phrases to canonical IDs from the data above.
9. Build typed payload and call the appropriate submit endpoint from `supportedActions` (the endpoint name and path are in each action entry).
10. If status is `pending_approval`, confirm to the user that the command has been queued. Approval/rejection happens from the Gatherly dashboard by an admin.
